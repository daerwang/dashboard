CREATE PROCEDURE IBMOB700.execute_aml_approval_118(IN i_requestId char(15), OUT o_status char(50)) 
LANGUAGE SQL 
BEGIN atomic
...
-- insert the logs
INSERT INTO XPERTV118.PAMLLOGCIF (
	AMLCBANKNU
	,AMLCCIFCOD
	,AMLCTYPLOG
	,AMLCSBTSUB
...
WHERE A.requestId = i_requestId
	...
	AND A.cifReference IN (
		SELECT B.AMLCCIFCOD
			...
			WHERE B.AMLCCIFCOD = A.cifReference
			AND B.AMLLFLAAPR = CASE B.AMLLFLARQD
			WHEN '1'
				THEN 'A'
			WHEN '2'
				THEN 'A'
